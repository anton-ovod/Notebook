...
def create_file(content="", filename="Untitled"):
    ...
    notebook.add(container, text=filename)
...
def current_tab_unsaved():
    current = get_text_widget()
    content = current.get("1.0", "end-1c")
    return hash(content) != text_contents[str(current)]
...
def show_about_info():
    messagebox.showinfo(
        title="About",
        message="This Text Editor is a simple tabbed text editor designed using Tkinter."
    )
...
file_menu.add_command(label="Close", command=close_current_tab, accelerator="Ctrl+Q")
...
root.bind("<Control-e>", lambda event: confirm_quit())
root.bind("<Control-q>", lambda event: close_current_tab())
root.bind("<Control-h>", lambda event: show_about_info())
